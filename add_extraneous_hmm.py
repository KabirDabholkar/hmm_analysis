...
def decoding(input_model,target_model,observations,name_to_save_plot="test.png"):
    train_observations,test_observations = train_test_split(observations)

    input_proba = predict_proba(input_model, train_observations, back_to_3D=False)
    target_proba = predict_proba(target_model, train_observations, back_to_3D=False)
    target_proba_sampled = batch_choice(target_proba)
    # model = LogisticRegression(multi_class='multinomial')
    models = [LogisticRegression(multi_class='multinomial'),MLPClassifier(hidden_layer_sizes=(100,)),MLPClassifier(hidden_layer_sizes=(200,))]
    ...
    ### testing
    input_proba = predict_proba(input_model, test_observations, back_to_3D=False)
    target_proba = predict_proba(target_model, test_observations, back_to_3D=False)
    # print(input_proba.shape,target_proba.shape)

    # pred_target = model.predict(input_proba)
