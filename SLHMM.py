...
class HMM(object):
    ...
    def __init__(self, num_hidden, num_observ,
        ...
        '''
        @num_hidden: int. Number of hidden states in the HMM.
        @num_observ: int. Number of observations in the HMM.
        ...
        if transition_matrix != None:
            ...
            self._transition_matrix = np.eye(num_hidden, dtype=float)
        ...
        if initial_dist != None:
            ...
            self._initial_dist = np.ones(num_hidden, dtype=float)
    ...
    def decode(self, sequence):
        ...
        '''
        t = len(sequence)
        prob_grids = np.zeros((t, self._num_hidden), dtype=float)
        path_grids = np.zeros((t, self._num_hidden), dtype=int)
        # Boundary case
        ...
        opt_hidden_seq = np.zeros(t, dtype=int)
    ...
    def generate_data(self, dsize, min_seq_len=3, max_seq_len=50):
        ...
        @dsize: int. Number of observation sequences to be generated
        @min_seq_len: int. Minimum length of each observation sequence, inclusive
        @max_seq_len: int. Maximum length of each observation sequence, exclusive
        '''
        data = []
    ...
    def _alpha_process(self, sequence):
        ...
        '''
        t = len(sequence)
        grids = np.zeros((t, self._num_hidden), dtype=float)
...
class EMHMM(HMM):
    ...
    def __init__(self, num_hidden, num_observ,
                 transition_matrix=None, observation_matrix=None, initial_dist=None):
        '''
        @num_hideen: int, number of hidden states in HMM.
        @num_observ: int, number of observations in HMM.
    ...
    def fit(self, sequences, max_iters=100, repeats=10, seq_length=100,
        ...
        @sequences: [np.array]. List of observation sequences, each observation
                    sequence can have different length
        @repeats (optional): int. Rerun the Expectation-Maximization algorithm
        ...
        for i in range(repeats):
            ...
            while iters < max_iters:
                iters += 1
                initial_dist = np.zeros(self._num_hidden, dtype=float)
                transition = np.zeros((self._num_hidden, self._num_hidden), dtype=float)
                observation = np.zeros((self._num_observ, self._num_hidden), dtype=float)
                for seq in short_sequences:
                    ...
                    for k in range(len(seq) - 1):
                        sigma_matrix = self._transition_matrix * \
                                       np.outer(self._observation_matrix[seq[k + 1], :] * beta_matrix[k + 1, :],
                                                alpha_matrix[k, :])
                        sigma_matrix /= np.sum(sigma_matrix)
...
class SLHMM(HMM):
    ...
    def __init__(self, num_hidden, num_observ,
        ...
        self._P_1 = np.zeros(self._num_observ, dtype=float)
        self._P_21 = np.zeros((self._num_observ, self._num_observ), dtype=float)
        self._P_3x1 = np.zeros((self._num_observ, self._num_observ, self._num_observ), dtype=float)
    ...
    def fit(self, sequences, rank_hyperparameter=None, verbose=False):
        ...
        self._Bx = np.zeros((self._num_observ, rank_hyperparameter, rank_hyperparameter), dtype=float)
        for index in range(self._num_observ):
...
class SLHMMwrapper(SLHMM):
    ...
    def __init__(self, num_hidden, num_observ,
                 transition_matrix=None, observation_matrix=None, initial_dist=None):
        super(SLHMMwrapper,self).__init__(num_hidden, num_observ,
                                    transition_matrix, observation_matrix, initial_dist)

    def preprocess(self,X: np.ndarray,lengths=None):
        seq = []
        if lengths is None:
            seq+=[X]
        lcount = 0
        for l in lengths:
            seq+=[np.array(X[lcount:lcount+l])]
            lcount += l
        return seq

    def fit(self,X: np.ndarray,lengths=None,rank_hyperparameter=None):
        seq = self.preprocess(X,lengths=lengths)
        super().fit(seq,rank_hyperparameter=rank_hyperparameter)

    def predict(self,X: np.ndarray,lengths=None):
        sequences = self.preprocess(X,lengths=lengths)
        logprobs = []
        for seq in sequences:
            logprobs += [self.predict(seq)]
        return logprobs

def main():
    model = SLHMM(2,2)
    seq = [np.random.choice(2, size=(70,)).astype(np.int64)]
    #model.fit(seq)
    #print(model.predict(seq[0]))
